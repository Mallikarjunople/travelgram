...
router.get('/addCity',async(req,res,next)=>{
  ...
  try{
      const allCity =await City.find({}).populate('popularcities','_id Title Tags Location Pictures');
      res.status(200).json({
          ...
          cities:allCity.map(allCity => {
              return{
                  ...
                  request:{
                      type:'GET',
                      url:'http://localhost:5000/admin/addCity/'+allCity._id
                  }
              }
          })
...
router.get('/addCity/:location',async(req,res,next)=>{
  ...
  try{
      const allCity =await City.find({Location:req.params.location}).populate('popularcities','_id Title Tags Location Pictures');
      res.status(200).json({
          count:allCity.length,
          cities:allCity.map(allCity => {
              return{
        ...
        await PCity.findByIdAndDelete(req.params.placeId,function(err,docs){
            if(err){
                ...
                res.status(200).json({
                    ...
                    request:{
                        type:'POST',
                        url:'http://localhost:5000/admin/addCity/popularPlace'
                    }
                });
            }
        });
